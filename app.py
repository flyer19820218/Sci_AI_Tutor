...
def create_student_pdf(row, output_dir, class_name="9年01班"):
...
# 標題
story.append(Paragraph(f"<b>《生物科第2回複習考》個人化診斷報告</b>", title_style))
...
header_data = [
    [Paragraph(f"<b>班級：</b>{class_name}", body_style), Paragraph(f"<b>座號：</b>{row['座號']} 號", body_style)],
...
story.append(Spacer(1, 6))
story.append(Paragraph(f"班級：{class_name}  姓名：{row['姓名']}  座號：{row['座號']}", body_style))
story.append(Spacer(1, 8))
...
with st.spinner("⏳ 正在生成 PDF，請稍候（約 10~30 秒）..."):
    ...
    st.success(f"🎉 成功產生 {success_count}/{len(df)} 位學生的個人化診斷報告！")
